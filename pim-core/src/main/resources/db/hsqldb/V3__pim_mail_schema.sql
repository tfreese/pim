...
CREATE UNIQUE INDEX MAIL_UNQ ON MAIL (ACCOUNT_ID, ID, FOLDER);
ALTER TABLE MAIL ADD CONSTRAINT MAIL_CK_ID CHECK(LENGTH(TRIM(ID)) > 0);
...
COMMENT ON COLUMN MAIL.ID IS 'Message-ID oder bei IMAP die UID der Mail';
