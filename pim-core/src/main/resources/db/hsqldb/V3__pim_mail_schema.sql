...
ALTER TABLE MAILFOLDER ADD CONSTRAINT MAILFOLDER_UNQ_FULLNAME_UID CHECK(LENGTH(TRIM(FULLNAME)) > 0);
ALTER TABLE MAILFOLDER ADD CONSTRAINT MAILFOLDER_UNQ_NAME_UID CHECK(LENGTH(TRIM(NAME)) > 0);
...
COMMENT ON COLUMN MAIL.FOLDER_ID IS 'Folder der Mail';
COMMENT ON COLUMN MAIL.MSG_NUM IS 'Message-Nummer der Mail im Folder';
COMMENT ON COLUMN MAIL.UID IS 'UID der Mail';
